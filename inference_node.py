#!/usr/bin/env python3
# (copyright) (c) 2026 I4A70 李軒杰
# Tatung University – I4210 AI實務專題

"""Inference node: runs YOLO26 TensorRT engine, publishes detections to MQTT.

Reads frames from a video file (or camera), runs detection with the fine-tuned
TensorRT engine (compiled inside the Docker image), and publishes bounding-box
...
def write_health():
    ...
    try:
        with open("/tmp/inference_health", "w", encoding="utf-8") as f:
...
def _open_source(source: str) -> cv2.VideoCapture:
    # Accept either a file path or a camera index ("0", "1", ...)
    if source.isdigit():
...
def main():
    ...
    parser.add_argument(
        "--source",
        default="/opt/data/test_video.mp4",
    ...
    cap = _open_source(str(args.source))
    if not cap.isOpened():
    ...
    while running:
        ret, frame = cap.read()
        if not ret:
            # Loop video for continuous testing (file inputs)
